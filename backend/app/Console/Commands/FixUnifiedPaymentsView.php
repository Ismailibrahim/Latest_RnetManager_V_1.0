...
class FixUnifiedPaymentsView extends Command
{
    protected $signature = 'fix:unified-payments-view';
    protected $description = 'Fix the unified_payments view to include property_name and unit_number';

    public function handle()
    {
        $this->info('Dropping existing view...');
        DB::statement('DROP VIEW IF EXISTS unified_payments');

        $this->info('Creating updated view with property_name and unit_number...');
...
SQL;
        ...
        $columns = DB::select('SHOW COLUMNS FROM unified_payments');
        $hasProperty = false;
        $hasUnitNumber = false;
        foreach ($columns as $col) {
            if ($col->Field === 'property_name') {
                $hasProperty = true;
                $this->info('✓ property_name column exists');
            }
            if ($col->Field === 'unit_number') {
                $hasUnitNumber = true;
                $this->info('✓ unit_number column exists');
            }
        }
        
        if (!$hasProperty) {
            $this->error('✗ property_name column MISSING');
        }
        if (!$hasUnitNumber) {
            $this->error('✗ unit_number column MISSING');
        }
        
        // Test with actual data
        $test = DB::table('unified_payments')->where('unit_id', 5)->first();
        if ($test) {
            $this->info("\nTest payment (unit_id=5):");
            $this->info("  unit_number: " . ($test->unit_number ?? 'NULL'));
            $this->info("  property_name: " . ($test->property_name ?? 'NULL'));
        } else {
            $this->warn('No payment found with unit_id=5 for testing');
        }
